...
```bash
python3 scripts/token-manager.py list
```

### Add Token

```bash
# 获取 Token: https://open.mcd.cn/mcp → 登录 → 控制台 → 激活
python3 scripts/token-manager.py add <名称> <token>

# 示例
python3 scripts/token-manager.py add Zoe 1kxNLFYT...LuqJ
```

### Multi-Token Management

```bash
python3 scripts/token-manager.py add 我自己 <token1>
python3 scripts/token-manager.py add 老妈 <token2>
python3 scripts/token-manager.py switch 老妈
python3 scripts/token-manager.py remove <名称>
```

Token 存储在 `~/.mcd-tokens.json`，`mcd_rpc.py` 会自动读取当前激活的 token。
