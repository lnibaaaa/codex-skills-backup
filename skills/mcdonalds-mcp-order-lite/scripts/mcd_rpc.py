...
# Auto-load token from token manager if not set via env
if not TOKEN:
    _token_file = os.path.expanduser("~/.mcd-tokens.json")
    if os.path.exists(_token_file):
        try:
            with open(_token_file) as _f:
                _data = json.load(_f)
                _active = _data.get("current", "") or _data.get("active", "")
                _tokens = _data.get("tokens", {})
                if _active and _active in _tokens:
                    TOKEN = _tokens[_active]
        except Exception:
            pass
